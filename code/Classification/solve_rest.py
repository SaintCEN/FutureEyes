...
# 数据加载
train = pd.read_excel('/tmp/pycharm_project_387/dataset/Training_Tag.xlsx')
test = pd.read_csv('/tmp/pycharm_project_387/outputs/Saint_ODIR.csv')
...
numeric_cols = train_df.drop(columns=['N','ID','Patient Age']).select_dtypes(include=['int', 'float']).columns
...
# 欠采样正常样本，使其数量近似于非正常样本的数量
n_non_normal = len(non_normal_samples)
balanced_normal = normal_samples.sample(n=int(n_non_normal/2), random_state=73)
# 合并并打乱顺序
...
class ODIRDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # 加载左右眼图像
        left_path = os.path.join('/tmp/pycharm_project_387/dataset/All/', self.df.iloc[idx]['Left-Fundus'])
        right_path = os.path.join('/tmp/pycharm_project_387/dataset/All/', self.df.iloc[idx]['Right-Fundus'])
        # 转为RGB
...
def train_model():
    ...
    best_model_path = '/tmp/pycharm_project_387/models/best_model_all.pth'  # 定义最佳模型保存路径
...
def predict():
    ...
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model = EfficientNet().to(device)
    model.load_state_dict(torch.load('/tmp/pycharm_project_387/models/best_model_all.pth'))
    model.eval()
    ...
    test.to_csv('/tmp/pycharm_project_387/outputs/SaintCHEN_ODIR.csv', index=False)
